...
async function bookAppointment(apiKey, { calendarId, locationId, contactId, startTime, name }) {
  ...
  const startUTC = start.toISOString(); // Convert to UTC so GHL doesn't misinterpret the offset
  const endTime = new Date(start.getTime() + 30 * 60 * 1000).toISOString();
  const payload = JSON.stringify({
    calendarId, locationId, contactId, startTime: startUTC, endTime,
...
function extractSlots(data, limit = 2, timeFilter = null) {
  const slots = [];
  ...
  for (const date of Object.keys(dates).sort()) {
    ...
    for (const slot of daySlots) {
      if (timeFilter === "afternoon") {
        const hour = new Date(slot).toLocaleString("es-ES", { hour: "numeric", timeZone: "Europe/Madrid", hour12: false });
        if (parseInt(hour) < 14) continue;
      }
...
function formatSlotsMessage(slots) {
  const formatted = slots.map((slot, i) => {
    ...
    const date = d.toLocaleDateString("es-ES", { weekday: "long", day: "numeric", month: "long", timeZone: "Europe/Madrid" });
...
async function sendSlotsAsButtons(apiKey, contactId, locationId, slots) {
  const buttons = slots.slice(0, 3).map((slot, i) => {
    const d = new Date(slot);
    const day = d.toLocaleDateString("es-ES", { weekday: "short", day: "numeric", month: "short", timeZone: "Europe/Madrid" });
    const time = d.toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Madrid" });
    return { type: "reply", reply: { id: String(i + 1), title: `${day} ${time}`.slice(0, 20) } };
  });
  const payload = JSON.stringify({
    type: "WhatsApp", contactId, locationId,
    message: "Elige un horario",
    interactive: {
      type: "button",
      body: { text: "¿Cuál te viene mejor?" },
      action: { buttons },
    },
  });
  return makeRequest({
...
module.exports = { sendMessage, getContact, getFreeSlots, getFreeSlotsByDate, bookAppointment, cancelAppointment, updateContact, sendSlotsAsButtons, extractSlots, formatSlotsMessage };
