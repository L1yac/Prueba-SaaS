...
function formatSlotsMessage(slots) {
  const formatted = slots.map((slot, i) => {
    ...
    return `${i + 1}. ${date} a las ${time}`;
  });
  return `Estos son los próximos horarios disponibles:\n\n${formatted.join("\n")}\n\n¿Cuál te viene mejor?`;
}
...
module.exports = { sendMessage, getContact, getFreeSlots, bookAppointment, cancelAppointment, extractSlots, formatSlotsMessage };
