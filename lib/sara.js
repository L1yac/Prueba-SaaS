...
async function getSaraDecision(clinic, lead, qualification, conversationHistory, forcedAction = null) {
  let systemPrompt = buildSystemPrompt(clinic, lead, qualification);

  if (forcedAction) {
    systemPrompt += `\n\n⚠️ INSTRUCCIÓN OBLIGATORIA: El lead acaba de pedir explícitamente ${forcedAction === "cancel_appointment" ? "CANCELAR" : "CAMBIAR"} su cita. Tu respuesta JSON DEBE tener action: "${forcedAction}". No preguntes confirmación, ejecuta la acción directamente.`;
  }
